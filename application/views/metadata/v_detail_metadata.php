            ...
            <tr>
                ...
                <td>
                    ...
                    <a class='btn btn-xs btn-danger <?php if ($this->session->userdata('role_id') == 2) {echo 'hidden';}?>'
                        onclick="del('<?=base_url('metadata/delete_hardware/'.$value->id.'/'.$kode.'/'.$id_aloptama.'/'.$jenis_aloptama) ?>' , '<?=$value->jenis_hardware?> Site <?=$kode?>')">
                        Delete</a>
                </td>
            </tr>
            <?php }?>
...
<script>
function del(url, pesan) {
    if (confirm("yakin ingin menghapus hardware " + pesan)) {
