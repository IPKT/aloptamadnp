<div class="row">
    ...
    <div class="col-sm-4">
        ...
        if ($this->session->flashdata('pesan')) {
            ...
            echo '  <a class="btn btn-xs btn-success" href="'.base_url('wrs"').'>Lihat Data</a> </div>';
        }
        
        ?>

        <?php echo form_open_multipart('wrs/update_data')?>
        <div class="form-group" hidden>
            <label for="">id wrs</label>
            <input class="form-control" name="id" placeholder="Nama Lokasi" value="<?=$wrs->id_wrs?>" required>
        </div>
        <div class="form-group">
            <label for="">Kode</label>
            <input class="form-control" name="kode" placeholder="Nama Site" required value="<?=$wrs->lokasi?>">
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <input class="form-control" name="detail_lokasi" placeholder="Detail Lokasi"
                value="<?=$wrs->detail_lokasi?>">
        </div>
        <div class="form-group">
            <label for="">Tipe</label>
            <select class="form-control" name="tipe" id="cars">
                <option <?php if ($wrs->tipe =="WRS20") {
                    echo "selected";
                }?> value="WRS20">WRS20</option>
                <option <?php if ($wrs->tipe =="WRS21") {
                    echo "selected";
                }?> value="WRS21">WRS21</option>
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <a href="<?=base_url('wrs/')?>"><button class="btn btn-danger">Kembali</button></a>
        </div>
