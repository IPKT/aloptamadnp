<div class="row">
    ...
    <div class="col-sm-4">
        ...
        if ($this->session->flashdata('pesan')) {
            ...
            echo '  <a class="btn btn-xs btn-success" href="'.base_url('int_reis"').'>Lihat Data</a> </div>';
        }
        
        ?>

        <?php echo form_open_multipart('int_reis/input_data')?>
...
L.marker([<?=$value->koordinat?>] <?php if ($value->kondisi_terkini =='ON' && $result=="Dengan Catatan") { ?>, {
    ...
    "<a class='btn btn-xs btn-warning' href='<?=base_url('int_reis/edit/'.$value->id_int_reis) ?>'> Edit</a> " +
    "<a class='btn btn-xs btn-danger' href='<?=base_url('int_reis/delete/'.$value->id_int_reis) ?>'> Delete</a> " +
    "</div>").addTo(map);
<?php    } ?>
