...
<div class="table-responsive m-3">
    ...
    <button class="btn btn-m btn-success" type="button" data-column="#column-action">Cetak Laporan PDF</button><span>   </span><button class="btn btn-m btn-success" type="button" data-kolom="#column-a" >Cetak Laporan Excel</button> <br> </br> <br> </br>
    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
        ...
        $int_reis = $CI->m_int_reis->allData(); 
        foreach($int_reis as $key => $value){?>
            <tr>
                ...
                <td><?=$jml = $CI->m_int_reis->ambil_jumlah_kunjungan($value->id_int_reis);?></td>
                <td><?php $tanggal= $CI->m_int_reis->tanggal_kunjungan_terbaru($value->id_int_reis);
                 $newDate = date("d-m-Y", strtotime($tanggal));  
                 echo $newDate;
                ?></td>
                
                <td><?php 
                $rekomendasi= $CI->m_int_reis->rekomendasi_kunjungan_terbaru($value->id_int_reis);
                if ($rekomendasi != NULL and $rekomendasi != "" ) {
                  # code...
                  $reko = str_replace("\n", '<br />', $rekomendasi->rekomendasi);
                  echo $reko;
                };
                if (($rekomendasi= $CI->m_int_reis->rekomendasi_kunjungan_terbaru($value->id_int_reis)) == "") {
                  # code...
                };?></td>


                <td><?php 
                $kerusakan= $CI->m_int_reis->kerusakan_kunjungan_terbaru($value->id_int_reis);
                if ($kerusakan != NULL and $kerusakan != "" ) {
                  # code...
                  // $ker = str_replace("\n", '<br />', $kerusakan->kerusakan);
                  echo $kerusakan->kerusakan;
                };
                if (($kerusakan= $CI->m_int_reis->kerusakan_kunjungan_terbaru($value->id_int_reis)) == "") {
                  # code...
                };?></td>
                <td>
                    <a class='btn btn-xs btn-success'
                        href='<?=base_url('int_reis/detail_int_reis/'.$value->id_int_reis) ?>'>
                        Detail</a>
                    <a class='btn btn-xs btn-warning <?php if ($this->session->userdata('role_id') == 2) {echo 'hidden';}?>'
                        href='<?=base_url('int_reis/edit/'.$value->id_int_reis) ?>'>
                        Edit</a>
                    <a class='btn btn-xs btn-danger hidden <?php if ($this->session->userdata('role_id') == 2) {echo 'hidden';}?>'
                    onclick="del('<?=base_url('int_reis/delete/'.$value->id_int_reis) ?>' , '<?=$value->kode?>')"> Delete</a>
                </td>
            </tr>
            <?php }?>
        </tbody>
...
L.marker([<?=$value->koordinat?>] <?php if ($value->kondisi_terkini =='ON' && $result=="Dengan Catatan") { ?>, {
    ...
    "<a class='btn btn-xs btn-danger hidden <?php if ($this->session->userdata('role_id') == 2) {echo 'hidden';}?>' onclick='del(`<?=base_url('int_reis/delete/'.$value->id_int_reis) ?>`,`<?=$value->kode?>`)'> Delete</a> " +
    "</div>").addTo(map);
<?php    } ?>
<?php $jml_int = $CI->m_int_reis->jumlah_int_reis();
          $jml_off = $CI->m_int_reis->jumlah_off();
          $jml_on = $CI->m_int_reis->jumlah_on(); ?>
...
legend.onAdd = function(map) {
    ...
    div.innerHTML += "<h4>Kondisi int_reismeter</h4>";
...
today = dd + '/' + mm + '/' + yyyy;
var text = "Laporan Intensity Reis " + today
doc.text(text, 12, 10)
doc.autoTable({
    html: '#dataTables-example'
})

doc.save('int_reis'+today+'.pdf')
column.toggleClass("hidden");
});
...
function ExportToExcel(type, fn, dl) {
       ...
       return dl ?
         ...
         XLSX.writeFile(wb, fn || ('IntReis.' + (type || 'xlsx')));
    }
