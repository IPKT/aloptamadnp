<div class="row">
    ...
    <div class="col-sm-4">
        ...
        if ($this->session->flashdata('pesan')) {
            ...
            echo '  <a class="btn btn-xs btn-success" href="'.base_url('int_reis/list_kunjungan"').'>Lihat Data</a> </div>';
        }
        
        $CI =& get_instance();
        $CI->load->model('m_int_reis');
        $int_reis = $CI->m_int_reis->detail_int_reis($kunjungan->id_int_reis)
        ?>
        <div class="form-group hidden">
            <button type="" class="btn btn-success" name="btn-check">Cek Lokasi</button>
        </div>
        <?php echo form_open_multipart('int_reis/update_data_kunjungan')?>
        <div class="form-group" hidden>
            <label for="">Kode</label>
            <select class="form-control" name="" id="cars">
                <?php foreach($site as $key => $value){?>
                <option value="<?=$value->id_int_reis?>:<?=$value->koordinat?>"><?=$value->kode?></option>
                <?php }?>
            </select>
        </div>
        <div class="form-group" hidden>
        ...
        <div class="form-group" hidden>
            <label for="">ID_int_reis</label>
            <input class="form-control" name="id_int_reis" placeholder="" required value="<?=$int_reis->id_int_reis?>">
        </div>
        <div class="form-group">
            <label for="">Kode</label>
            <input class="form-control" name="kode" placeholder="" required value="<?=$int_reis->kode?> (<?=$int_reis->lokasi?>)">
        </div>
        <div class="form-group">
        ...
        <?php echo form_close()?>
        <a href="<?=base_url('int_reis/list_kunjungan')?>"><button class="btn btn-success">Kembali</button></a>
...
var curLocation = [<?php echo $int_reis->koordinat?>];
