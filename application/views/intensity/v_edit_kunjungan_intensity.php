<div class="row">
    ...
    <div class="col-sm-4">
        ...
        $intensity = $CI->m_intensity->detail_intensity($kunjungan->id_intensity)
        ?>
        <div class="form-group hidden">
            <button type="" class="btn btn-success" name="btn-check">Cek Lokasi</button>
        </div>
        <?php echo form_open_multipart('intensity/update_data_kunjungan')?>
        <div class="form-group" hidden>
            <label for="">Kode</label>
            <select class="form-control" name="" id="cars">
                <?php foreach($site as $key => $value){?>
                <option value="<?=$value->id_intensity?>:<?=$value->koordinat?>"><?=$value->kode?></option>
                <?php }?>
            </select>
        </div>
        <div class="form-group" hidden>
        ...
        <div class="form-group" hidden>
            <label for="">ID_Intensity</label>
            <input class="form-control" name="id_intensity" placeholder="" required
                value="<?=$intensity->id_intensity?>">
        </div>
        <div class="form-group">
            <label for="">Kode</label>
            <input class="form-control" name="kode" placeholder="" required value="<?=$intensity->kode?>">
        </div>
        <div class="form-group">
...
var curLocation = [<?php echo $intensity->koordinat?>];
