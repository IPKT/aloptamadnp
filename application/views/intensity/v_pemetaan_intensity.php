...
<div class="table-responsive m-3">
    ...
    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
        <thead>
            ...
            $total_kunjungan = $CI->m_intensity->total_kunjungan(); 
        ...
        $CI =& get_instance();
        $CI->load->model('m_intensity');
        $intensity = $CI->m_intensity->allData(); 
        foreach($intensity as $key => $value){?>
            <tr>
                <td><?=$no++?></td>
                <td><a href="<?=base_url('intensity/detail_intensity/'.$value->id_intensity) ?>"><?=$value->kode?></a>
                </td>
                <td><?=$value->lokasi?></td>
                <td class="hidden"><?=$value->kondisi_terkini?></td>
                <td><?=$jml = $CI->m_intensity->ambil_jumlah_kunjungan($value->id_intensity);?></td>
                <td><?php $tanggal= $CI->m_intensity->tanggal_kunjungan_terbaru($value->id_intensity);
                    $newDate = date("d-m-Y", strtotime($tanggal));  
                    echo $newDate;  ?></td>

                <td><?php 
                $rekomendasi= $CI->m_intensity->rekomendasi_kunjungan_terbaru($value->id_intensity);
                if ($rekomendasi != NULL and $rekomendasi != "" ) {
                  # code...
                  $reko = str_replace("\n", '<br />', $rekomendasi->rekomendasi);
                  echo $reko;
                };
                if (($rekomendasi= $CI->m_intensity->rekomendasi_kunjungan_terbaru($value->id_intensity)) == "") {
                  # code...
                };?></td>


                <td><?php 
                $kerusakan= $CI->m_intensity->kerusakan_kunjungan_terbaru($value->id_intensity);
                if ($kerusakan != NULL and $kerusakan != "" ) {
                  # code...
                  // $ker = str_replace("\n", '<br />', $kerusakan->kerusakan);
                  echo $kerusakan->kerusakan;
                };
                if (($kerusakan= $CI->m_intensity->kerusakan_kunjungan_terbaru($value->id_intensity)) == "") {
                  # code...
                };?></td>
                ...
                <td>
                    <a class='btn btn-xs btn-success'
                        href='<?=base_url('intensity/detail_intensity/'.$value->id_intensity) ?>'> Detail</a>
                    <a class='btn btn-xs btn-warning <?php if ($this->session->userdata('role_id') == 2) {echo 'hidden';}?>'
                        href='<?=base_url('intensity/edit/'.$value->id_intensity) ?>'>
                        Edit</a>
                    <a class='btn btn-xs btn-danger hidden <?php if ($this->session->userdata('role_id') == 2) {echo 'hidden';}?>'
                        onclick="del('<?=base_url('intensity/delete/'.$value->id_intensity) ?>' , '<?=$value->kode?>')">
                        Delete</a>
                </td>
            </tr>
            <?php }?>
...
L.marker([<?=$value->koordinat?>] <?php if ($value->kondisi_terkini =='ON' && $result=="Dengan Catatan") { ?>, {
    ...
    "<a class='btn btn-xs btn-danger <?php if ($this->session->userdata('role_id') == 2) {echo 'hidden';}?>' onclick='del(`<?=base_url('intensity/delete/'.$value->id_intensity) ?>`,`<?=$value->kode?>`)'> Delete</a> " +
    "</div>").addTo(map);
<?php    } ?>
<?php $jml_int = $CI->m_intensity->jumlah_intensity();
          $jml_off = $CI->m_intensity->jumlah_off();
          $jml_on = $CI->m_intensity->jumlah_on(); ?>
