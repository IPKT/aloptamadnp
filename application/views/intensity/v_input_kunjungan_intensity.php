<div class="row">
    ...
    <div class="col-sm-4">
        ...
        <?php echo form_open_multipart('intensity/input_data_kunjungan')?>
        <div class="form-group">
            <label for="">Kode</label>
            <select class="form-control" name="id_intensity" id="cars">
                <option selected disabled value="">Pilih Site</option>
                <?php foreach($site as $key => $value){?>
                <option value="<?=$value->id_intensity?>:<?=$value->koordinat?>"><?=$value->kode?> (<?=$value->lokasi?>)
                </option>
                <?php }?>
            </select>
        </div>
