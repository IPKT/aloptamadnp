<div class="row">
    ...
    <div class="col-sm-4">
        ...
        if ($this->session->flashdata('pesan')) {
            ...
            echo '  <a class="btn btn-xs btn-success" href="'.base_url('acc_noncolo/list_kunjungan"').'>Lihat Data</a> </div>';
        }
        
        $CI =& get_instance();
        $CI->load->model('m_acc_noncolo');
        $acc_noncolo = $CI->m_acc_noncolo->detail_acc_noncolo($kunjungan->id_acc_noncolo)
        ?>
        <div class="form-group">
            <button type="" class="btn btn-success" name="btn-check">Cek Lokasi</button>
        </div>
        <?php echo form_open_multipart('acc_noncolo/update_data_kunjungan')?>
        <div class="form-group" hidden>
            <label for="">Kode</label>
            <select class="form-control" name="" id="cars">
                <?php foreach($site as $key => $value){?>
                <option value="<?=$value->id_acc_noncolo?>:<?=$value->koordinat?>"><?=$value->kode?></option>
                <?php }?>
            </select>
        </div>
        <div class="form-group" hidden>
        ...
        <div class="form-group" hidden>
            <label for="">ID_acc_noncolo</label>
            <input class="form-control" name="id_acc_noncolo" placeholder="" required
                value="<?=$acc_noncolo->id_acc_noncolo?>">
        </div>
        <div class="form-group">
            <label for="">Kode</label>
            <input class="form-control" name="kode" placeholder="" required value="<?=$acc_noncolo->kode?>">
        </div>
        <div class="form-group">
