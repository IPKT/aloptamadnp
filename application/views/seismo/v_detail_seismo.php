<div class="row">
    ...
    <div class="col-sm-5">
        ...
        <table class="table table-striped table-hover">
            <tbody>
                ...
                <tr>
                    <td>Detail Lokasi</td>
                    <td>:</td>
                    <td><?=$seismo->detail_lokasi?></td>
                </tr>
                <tr>
                    <td>Koordinat</td>
                    <td>:</td>
                    <td><?=$seismo->koordinat?></td>
                </tr>
                <tr>
                    <td>Nama PIC</td>
                    <td>:</td>
                    <td><?=$seismo->nama_pic?></td>
                </tr>
                <tr>
                    <td>Kontak PIC</td>
                    <td>:</td>
                    <td><?=$seismo->kontak_pic?></td>
                </tr>
                <tr>
                    <td>Catatan Teknisi</td>
                    <td>:</td>
                    <td><?php 
                if ($seismo->catatan != NULL and $seismo->catatan != "" ) {
                  # code...
                  $catatan = str_replace("\n", '<br />', $seismo->catatan);
                  echo $catatan;
                };
        ...
        <a href="<?=base_url('seismo')?>"><button class="btn btn-success">Pemetaan</button></a>
    </div>
...
<div class="table-responsive m-3">
    ...
    <table class="table table-striped table-bordered table-hover" id="dataTables-example" >
        ...
        $kunjungan = $CI->m_seismo->kunjungan_site_tertentu($seismo->kode); 
            ...
            <tr>
                ...
                <td>
                    <a class='btn btn-xs btn-success'
                        href='<?=base_url('seismo/detail_kunjungan/'.$value->id_kunjungan) ?>'>
                        Detail</a>
                    <a class='btn btn-xs btn-warning <?php if ($this->session->userdata('role_id') != 1 and $this->session->userdata('id_user') != $value->id_author )  {echo 'hidden';}?>'
                        href='<?=base_url('seismo/edit_kunjungan/'.$value->id_kunjungan) ?>'>
                        Edit</a>
                    <a class='btn btn-xs btn-danger <?php if ($this->session->userdata('role_id') == 2) {echo 'hidden';}?>'
                        onclick="del('<?=base_url('seismo/delete_kunjungan/'.$value->id_kunjungan) ?>' , '<?=$value->kode?>')">
                        Delete</a>
                </td>
            </tr>
            <?php }?>
